...
{   
    ...
    public function referralaccountAction()
    {
    	...
    	{            
            if (Mage::getStoreConfig('extras/virtualoffice/activate') == 1){
                $customer_id = (int)Mage::getSingleton('customer/session')->getCustomer()->getId();                
            	$customer = Mage::getSingleton('affiliate/affiliatecustomers')->load($customer_id);
                
                if ($customer->getVirtualOffice() == 1){
                    $this->loadLayout(); 
                    ...
                    $this->_redirect('affiliate/index/activate/');
                }
            }
            else{
                $this->loadLayout(); 
    ...
    public function activateAction(){
        ...
        $products = explode(",",  Mage::getStoreConfig('extras/virtualoffice/products'));        
        foreach ($sale as $order) {      
            $items = $order->getAllVisibleItems();           
            foreach($items as $item){
                $single = $item->getData();
                if (array_search($single['sku'], $products) >= 0){ 
                    $customer->setVirtualOffice(1);
                    $customer->save();
                    $this->_redirect('affiliate/index/referralaccount/');
                }
            }
    ...
    {   
    	...
    	$auto_withdrawn = (int)$this->getRequest()->getPost('auto_withdrawn');
    	$payment_release_level = (double)$this->getRequest()->getPost('payment_release_level');
    	$affiliate_network = $this->getRequest()->getPost('check_affiliate_network');
    	$bank_name = '';
    	$name_account = '';
    	$bank_country = '';
    	$swift_bic = '';
    	...
    	if($check_affiliate) 
    	{
            ...
            if($getway_withdrawn != 'banktransfer' && $getway_withdrawn != 'check') 
            {
                $collectionFilter = Mage::getModel('affiliate/affiliatecustomers')->getCollection()->addFieldToFilter('payment_email',$payment_email);
            ...
            $customers = Mage::getModel('affiliate/affiliatecustomers')->getCollection()->addFieldToFilter('customer_id',$customer_id);
            if(sizeof($customers) > 0)
            {
                ...
                $referral_edit->setAccountNumber($account_number);
                $referral_edit->setReAccountNumber($re_account_number);
                $referral_edit->setANetwork($affiliate_network);
				//$referral_edit->setReferralSite($referral_site);
                $referral_edit->save();
            }
            ...
            else if(sizeof($customers) == 0)
            {
                ...
                if($referral_site) {
                    ...
                    foreach($sites as $url) {
                        ...
                        $webwsiteModel->setData($siteItem);
                        $websiteModel->save();
                    }
                }
            ...
            else if($active == MW_Affiliate_Model_Statusactive::ACTIVE)
            {
                ...
                Mage::helper('affiliate')->setMemberDefaultGroupAffiliate($customer_id,$store_id,$check_affiliate);
    ...
    public function affiliatebinaryAction() {
    	$store_id = Mage::app()->getStore()->getId();
    	if(!Mage::helper('affiliate/data')->getEnabledStore($store_id)){
            $this->norouteAction();
            return;
...
//            ->addFieldToFilter('customer_invited',$customer_id)
        ...
        if (sizeof($hold->getData())>0){      
            ...
            if (sizeof($check->getData())>0){
                ...
                $network =  Mage::helper('affiliate_binary')->binaryNetwork($customer_parent[0]['customer_id'], '', $parent, ($level));
	...
	$name = Mage::getModel('customer/customer')->load($customer_id)->getName();
        $name_id = $customer_id;// Mage::getModel('customer/customer')->load($customer_id)->getId();
    ...
    public function exportAction(){
        ...
        foreach($data as $row) {
            if(!$flag) {
                echo implode("\t", array_keys($row->getData())) . "\r\n";
                $flag = true;
            }
            array_walk($row->getData(), 'cleanData');
            echo implode("\t", array_values($row->getData())) . "\r\n";
        }        
    }
