<?php
class MW_Affiliate_Block_Adminhtml_Affiliateexpired_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
    public function __construct()
    {
        parent::__construct();
        $this->setId('affiliateexpired');
    ...
    {        
        ...
        $collection->getSelect()->join(array('mw'=>'mw_affiliate_customers'),'main_table.customer_id = mw.customer_id',array())
                ->where('mw.anetwork = ?', 2);       
        
        $collection->addFieldToFilter('mw.customer_id', array('nin' => array($preferred)));
        $collection->getSelect()->order(array('main_table.expired_package DESC'));
    ...
    protected function _prepareColumns()
    {
        ...
        $this->addColumn('group_name', array(
            ...
            $this->addColumn('expired_package', array(
          	'header'    => Mage::helper('affiliate')->__('Expired'),
          	'align'     =>'left',
                'width'     => '25px',
     	));
...
//        $this->getCollection()->getSelect()->where("main_table.customer_id in (?)",$customer_ids);
    ...
    {
        return $this->getUrl('*/*/editexpired', array('id' => $row->getCustomerId()));
    }
