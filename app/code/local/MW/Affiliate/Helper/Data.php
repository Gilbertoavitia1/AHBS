...
{  
    ...
    protected $frontal_network_table = array();
    protected $tree_network_table = array();
    protected $affiliate_network_table = array();
    protected $_frontalResult = array();
    protected $_treeResult = array();
    protected $_affiliateesult = array();
    ...
    public function insertCustomerCredit($customer_id)
    {
        ...
        if(!($customerCredit->getId())) 
        {
            ...
            $customerData = array('customer_id'=>$customer_id,'credit'=>0);
	...
	public function getReferralSponsorByCheckout()
    {
    	return Mage::getSingleton('checkout/session')->getReferralSponsor();
    }
    ...
    {   
    	...
    	$store_id = $customerData['website_id'] == 0 ? 1 : $customerData['website_id'];       
        ...
        $sql = "SELECT * FROM mw_affiliate_code WHERE store_id = {$store_id}";
        $query = $read->query($sql)->fetch();
        $referral_code = $query['referral_code'];
        $new_code = $referral_code + 1;
        
        $i = 0;
        
        while ($i == 0) {
            $affiliate_customers_filter = Mage::getModel('affiliate/affiliatecustomers')
                ->getCollection()
                ->addFieldToFilter('referral_code', $new_code);
        ...
        $sql = "UPDATE mw_affiliate_code SET referral_code='{$new_code}' WHERE store_id={$store_id}";
    	$write->query($sql);
        
        $referral_code_new = $new_code;
    ...
    public function setReferralSponsor($customer_id)
    {   
    	$affiliate_customers = Mage::getModel('affiliate/affiliatecustomers')->load($customer_id);
    	$store_id = $affiliate_customers->getStoreId();
    	$length = (int)Mage::helper('affiliate/data')->getLengthReferralCodeStore($store_id);
    	$i = 0;
		$referral_sponsor = $this->rand_str($length);
		while ($i == 0) {
		       $affiliate_customers_filter = Mage::getModel('affiliate/affiliatecustomers')->getCollection()
								                        ->addFieldToFilter('referral_sponsor', $referral_sponsor);
		       if(sizeof($affiliate_customers_filter) > 0) {
			       $i = 0;
			       $referral_sponsor = $this->rand_str($length);
		       } else {
		...
		$referral_code_new = $referral_sponsor;
		$affiliate_customers->setReferralCode($referral_code_new)->save();
    }
    ...
    public function getCustomerIdByReferralSponsor($referral_sponsor, $cokie)
    {   
    	$result = $cokie;
    	if(isset($referral_sponsor) && $referral_sponsor != ''){
            $collectionCustomers = Mage::getModel('affiliate/affiliatecustomers')->getCollection()
                            ->addFieldToFilter('referral_code',$referral_sponsor);
            if(sizeof($collectionCustomers) >0){
                foreach ($collectionCustomers as $collectionCustomer) {
                    $customer_id = $collectionCustomer ->getCustomerId();
                    $active = $collectionCustomer ->getActive();
...
//        $website_id = Mage::app()->getWebsite()->getId();
	    ...
	    if(sizeof($collectionCustomers) > 0){
	    	foreach ($collectionCustomers as $collectionCustomer) {
//	    		$code_website = Mage::getModel('customer/customer')->load($collectionCustomer->getCustomerId())->getWebsiteId();
//                        if ($code_website == $website_id){        
                            $active = $collectionCustomer->getActive();
                            $status = $collectionCustomer->getStatus();
                            break;
//                        }
    ...
    public function checkReferralSponsor($referral_sponsor)
    {   
    	$result = 0;
//        $website_id = Mage::app()->getWebsite()->getId();
    	$collectionCustomers = Mage::getModel('affiliate/affiliatecustomers')->getCollection()
...
//                        }
    ...
    public function checkCustomerUrl($customer_url)
    {   
    	$result = 0;
    	$collectionCustomers = Mage::getModel('affiliate/affiliatecustomers')->getCollection()
	...
	public function checkReferralSponsorCart($referral_sponsor)
    {   
    	$result = 0;
    	$customer_id = (int)Mage::getSingleton("customer/session")->getCustomer()->getId();
    	$collectionCustomers = Mage::getModel('affiliate/affiliatecustomers')->getCollection()
	    ...
	    if(sizeof($collectionCustomers) >0){
	    	foreach ($collectionCustomers as $collectionCustomer) {
	    		$customer_id_referral_sponsor = $collectionCustomer ->getCustomerId();
	    		$active = $collectionCustomer ->getActive();
    			$status = $collectionCustomer ->getStatus();
    			break;
	    	}
	    	if($active == MW_Affiliate_Model_Statusactive::ACTIVE && $status == MW_Affiliate_Model_Statusreferral::ENABLED){
	            $result = 1;
	            if(isset($customer_id) && $customer_id == $customer_id_referral_sponsor)	$result = 0;	
	        }
	    }
    	return $result;
    }
    ...
    public function setTotalMemberProgram()
    {
        ...
        if(sizeof($collectionPrograms) >0){
            foreach ($collectionPrograms as $collectionProgram) {
                ...
                $groupPrograms = Mage::getModel('affiliate/affiliategroupprogram')->getCollection()->addFieldToFilter('program_id',$program_id);
                if(sizeof($groupPrograms) >0){
                    foreach ($groupPrograms as $groupProgram) {
                        $group_id = $groupProgram ->getGroupId();
                        $customerPrograms = Mage::getModel('affiliate/affiliategroupmember')->getCollection()->addFieldToFilter('group_id',$group_id);
    ...
    public function setMemberDefaultGroupAffiliate($customer_id, $store_id=null, $check_affiliate)
    {
    	if($store_id == null) {
            $store_id = Mage::app()->getStore()->getId();
    	}
        
        if ($check_affiliate == 2)
            $group_id = $this->getGroupAffiliateStore($store_id);
        else{            
    	...
    	$data = array('customer_id' => $customer_id, 'group_id'	=> $group_id);    	
    	Mage::getModel('affiliate/affiliategroupmember')->setData($data)->save();
    }
	...
	public function getGroupAffiliateStore($store_id)
	{
		return Mage::getStoreConfig('affiliate/config/affiliate_group',$store_id);
	}
...
/*-------------------------------------------------------------------------------------------------------------------*/
    ...
    {   
        ...
        if($customer_id) {
            $collection = Mage::getModel('affiliate/affiliatecustomers')->getCollection()
                ...
                ->addFieldToFilter('status ',MW_Affiliate_Model_Statusreferral::ENABLED)
                ->addFieldToFilter('active',MW_Affiliate_Model_Statusactive::ACTIVE)
                ->addFieldToFilter('anetwork',array('nin' => array(2)));
    ...
    public function getFrontalParents($code, $customer_id)
    {   
        $result = array();
        if($code) {
            $collection = Mage::getModel('affiliate/affiliatecustomers')->getCollection()
    ...
    public function getSponsorParents($code, $customer_id)
    {   
        $result = array();
        if($code) {
            $collection = Mage::getModel('affiliate/affiliatecustomers')->getCollection()
    ...
    public function getPosition($customer_id){
        $customer = Mage::getModel('affiliate/affiliatecustomers')->load($customer_id);
        $flag = 0;
        $left = $customer->getLeftLeg();
        $right = $customer->getRightLeg();
    ...
    public function treeNetwork($customer_id, $referral, $parent_id, $i){
        $this->treeNetworkTable($customer_id, $referral, $parent_id, $i);
    ...
    public function affiliateNetwork($customer_id, $referral,$i){
        $this->affiliateNetworkTable($customer_id, $referral, null, 0);
        
        $array = $this->_affiliateResult;
        foreach ($array as $key => $item) { 
            $total = $item['vg'];
            $id = $item['id'];
            foreach ($array as $k => $child) {
    ...
    public function frontalNetwork($customer_id, $referral, $parent_id, $i){
        $this->frontalNetworkTable($customer_id, $referral, $parent_id, $i);
        
        $array = $this->_frontalResult;
        foreach ($array as $key => $item) { 
            $total = $item['vg'];
            $id = $item['id'];
            foreach ($array as $k => $child) {
    ...
    public function frontalNetworkTable($customer_id, $referral, $parent_id, $i){
        if(!in_array($customer_id, $this->frontal_network_table)) {
            $this->frontal_network_table[] = $customer_id;
            
            $vp = 0;
            $vg = 0;

            $tmp = $this->networkInfo($customer_id, $parent_id, $i, 1);
            
            if (sizeof($tmp)>0){
                $vp = $tmp['vp'];
                $childs = $this->getFrontalParents($tmp['code'], $customer_id); 

                if(sizeof($childs) > 0) {
                    $i = $i + 1 ;
                    foreach($childs as $child) {
                        $vg = $vg + $this->frontalNetworkTable($child, $tmp['name'], $tmp['code'], $i);
                    }
                }
                
                $tmp['vg'] = $vg;       
                
                $this->_frontalResult[] = $tmp;
            }            
        }  
    ...
    public function affiliateNetworkTable($customer_id, $referral, $parent_id, $i){
        if(!in_array($customer_id, $this->affiliate_network_table)) {
            $this->affiliate_network_table[] = $customer_id;
            ...
            if (sizeof($tmp)>0){
                $vp = $tmp['vp'];
                $childs = $this->getAffiliateParents($customer_id); 
                if(sizeof($childs) > 0) {
                    $i = $i + 1 ;
                    foreach($childs as $child) {
                        $vg = $vg + $this->affiliateNetworkTable($child, $tmp['name'], $tmp['code'], $i);
                    }
                }
                
                $tmp['vg'] = $vg;       
                
                $this->_affiliateResult[] = $tmp;
            }            
        }  
    ...
    public function treeNetworkTable($customer_id, $referral, $parent_id, $i){
        if(!in_array($customer_id, $this->tree_network_table)) {
            ...
            if (sizeof($tmp)>0){
                $vp = $tmp['vp'];
                $childs = $this->getSponsorParents($tmp['code'], $customer_id); 

                if(sizeof($childs) > 0) {
                    $i = $i + 1 ;
                    foreach($childs as $child) {
                        $vg = $vg + $this->treeNetworkTable($child, $tmp['name'], $tmp['code'], $i);
                    }
                }
    ...
    public function networkInfo($customer_id, $parent_id, $i, $type){
        ...
        if (!($group_name == "CLIENTE PREFERENTE")){   
            ...
            $array = array(
                ...
                'title'             => $name,
                'name'              => $name,
                'referral'          => "Colocado: ".$referral_name,
                'sponsor_name'      => "Directo: ".$sponsor_name,
                'parent'            => $parent_id,
                'sponsor'           => $sponsor,
